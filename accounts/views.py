...
def login(request):
    ...
    if not user:
        messages.error(request, 'Usuario e/ou senha inválido(s).')
        return render(request, 'accounts/login.html')
    else:
...
def register(request):
    ...
    if len(usuario) < 6:
        messages.error(request, 'Usuário deve ter 6 caracteres ou mais.')
        return render(request, 'accounts/register.html')
...
def dashboard(request):
    ...
    if not form.is_valid:
        messages.error(
            request, 'Ops, ocorreu um erro, verifique e tente novamente...')
